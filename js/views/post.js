...
Views.post = {
      ...
      <div class="form-section">
        ...
        <div class="form-group">
          ...
          <div id="f-tag-other-wrap" class="tag-other-input" style="display: ${s.showOtherInput ? 'block' : 'none'};">
            <input id="f-tag-other" class="form-input" type="text" placeholder="その他のタグ(カンマ区切り)" value="${escapeHtml(s.otherTags)}">
          </div>
        </div>
  ...
  async handleFileSelect(files) {
    for (const file of files) {
      ...
      const isAllowed = this.ALLOWED_MIME_TYPES.indexOf(file.type) !== -1 || file.type.startsWith('image/');
      if (!isAllowed) {
